...
- **所有会话标题都显示成「新会话」**（真机上 121 行里有 119 行中招）。两个各自独立的 bug
  叠在一起，只修一个都还会复现：
  1. **扩展侧的行内投影被忽略**：`session/list` 的每一行本来就自带
     `projections.values.title`，但扩展构建 `init` 时只读控制流的 `titles` 缓存。
     而控制流的 projection 切割是**稀疏**的 —— 只有该宿主进程里挂载了投影单元的会话才有
     切割（实测 245 个会话只有 10 条），所以初次连接的缓存几乎是空的，110 个标题被丢掉。
     现在优先采用行内值，仅当行内**完全没有** `title` 键时才回退到缓存。
  2. **store 把「缺切割」当成「没有标题」**：`session/control` 的基线被当作对每个会话都完整的
     全量切割，于是基线里没提到的会话一律被重算成 `title: null`，把 `init` 刚装好的标题
     又擦掉。基线实际是稀疏的，现在只重算**基线明确提到**的会话；某个会话的投影单元确实
     被卸载时，宿主会给出一条**存在**且 `title: null` 的切割，语义依然准确。

  这两个 bug 合起来正好解释了「重载后闪一下标题、随即全变成新会话」。

### 测试

- 单元测试 `186` 项（新增 5 项回归）：`tests/bridge-init.test.ts` 用进程内假宿主驱动**真实
  Bridge**，把控制基线故意留空，断言行标题仍来自 `session/list`；`tests/todo-fixes.test.ts`
  新增稀疏基线与空基线两条 store 回归。
- 为此让 `esbuild --tests` 也把 `vscode` 别名到已入库的 `tests/e2e/vscode-stub.ts`（e2e 早已
  这么做）：扩展模块在**运行时**调用 `vscode.window.show*Message`，此前没有任何单测能驱动 Bridge。
- E2E `26 通过 / 0 失败 / 1 跳过`；`tsc` 两个配置均 0 错误。
