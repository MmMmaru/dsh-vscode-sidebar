...
### 08-25 0.1.1 设置与会话管理窗口 3/4 尺寸及 16px 圆角全局同步
- 优化与落地：
  1. **设置窗口尺寸调整至 3/4**：`settings-dialog` 设置为 `width: 75%`，`height: 75%` 居中展示；
  2. **会话管理窗口尺寸调整至 3/4**：`chat-list-dropdown` 设置为 `width: 75%`，`height: 75vh` 居中展示；
  3. **圆角全面对齐 16px**：所有卡片与浮层（设置弹窗、会话下拉、确认模态、审批卡片、计划审查卡片、GoalBar、QueueDock、SubagentDock、TodoPanel）统一应用 16px 圆角。
- 测试：单测全量 118/118 绿，打包 `dsh-vscode-sidebar-0.1.1.vsix` 并安装至本地 VS Code。
...
- 用户定稿：代码块恢复最初的整块淡灰底（`.md-codeblock` background 回到 `color-mix(in srgb, var(--dsh-fg) 6%, transparent)`，无边框圆角不变）——透明态与 #1b1d21 深面板两版实验均被否，最终回到 0.0.12 原方案；斑马纹删除与 file-ref 兜底保持不变。单测契约同步为灰底断言。
- 测试：单测 98 绿；VSIX 重打安装。文档（CHANGELOG/TODO R2）更新至最终态。

### 08-19 0.0.12 返工三轮（条纹真凶：表格斑马纹）
- 深色面板复验被否（"反而变成黑色了，还是有条纹"），先回退面板实验为透明态并与整轮返工一起提交（b77ff3e，14 files）。随后实证排查：① esbuild 打包真实 MarkdownBlock 喂 13 种模型输出样本（无语言围栏/CRLF/未闭合/围栏内反引号·路径·表格·标题/缩进/波浪线等）——全部正确解析为 .md-codeblock，"围栏漏解析成正文"假设排除；② 请用户确认形态=「整行交替色带」+ 贴出实际消息样本（代码讲解型重 markdown 输出，标题/表格/text 块混排）——全样式表唯一能产生整行交替色带的元素只有表格斑马纹 `tbody tr:nth-child(even)`。修：删除斑马纹规则，表格统一底色（表头保留 --dsh-bg-raised 淡底，行间仅 1px 边框），并加防回归契约（nth-child 斑马规则禁回归）。
- 测试：单测 98 绿；VSIX 重打安装待验收。若重载后仍见色带，则升级为环境级问题（主题/GPU 合成），需截图定位。
